...
module.exports = {
  list : function(request,response){
    db.query(`SELECT count(*) as total FROM board`,function(error, nums){
      ...
      db.query(`SELECT * FROM board ORDER BY date desc, boardid LIMIT ? OFFSET ?`,
  ...
  create_process : function(request,response){
    ...
    request.on('end', function() {
      ...
      db.query(`INSERT INTO board (loginid, name, password, date, content,title) VALUES(?, ?, ?, ?, ?, ?)`,
        [post.id, post.nmh, post.passwd, dateOfEightDigit(),post.content, post.title], function(error, result) {
        if(error) {
          throw error;
        }
